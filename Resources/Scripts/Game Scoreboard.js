...
const casterSize = '44px';
const twitterSize = '40px';
	...
	if (startup) {
		...
		if (allowIntro) {
			...
			if (p1Score + p2Score == 0) { //if this is the first game, introduce players
				...
				p1IntroEL.style.fontSize = '50px'; //resize the font to its max size
				resizeText(p1IntroEL); //resize the text if its too large
				p2IntroEL.style.fontSize = '50px';
...
function updatePlayerName(wrapperID, nameID, teamID, pName, pTeam) {
	...
	let fontSize = '115px';
	fontSize = pName == 'Lengüitas' ? '50px' : fontSize;
	nameEL.style.fontSize = fontSize; //set original text size
	nameEL.textContent = pName; //change the actual text
	const teamEL = document.getElementById(teamID);
	fontSize = pTeam == 'RetroSHFFL' ? '40px' : '105px';
	teamEL.style.fontSize = fontSize;
